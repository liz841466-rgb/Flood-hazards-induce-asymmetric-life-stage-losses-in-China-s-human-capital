#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archived notebook note for 06_legacy_city_gumbel_exposure_version.
...
This public compact repository keeps notebook-export code for workflow auditability. Restricted raw data and derived individual-level panels are not included."""
    ...
    # Original notebook comment normalized for the public code archive.
    out_parquet = OUT_DIR / f"city_flood_Tall_{window}y_1980_2020.parquet"
    out_excel   = OUT_DIR / f"city_flood_Tall_{window}y_1980_2020.xlsx"

    # Original notebook comment normalized for the public code archive.
    dfN.to_parquet(out_parquet, index=False)
    print("[INFO] Notebook progress message.")

    # Excel output note.
    dfN.to_excel(out_excel, index=False)
    print("[INFO] Notebook progress message.")
