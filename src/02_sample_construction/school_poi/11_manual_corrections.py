#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archived notebook note for 11_manual_corrections.
...
def refine_L3_with_ntl_pop(
    years_clcd,
    build_year,
    CI,
    case_type,
    meta,
    allowed_case_types=("A_CLCD_left", "C_urban_N", "D_NoDate"),
    max_year=None
):
    """Archived notebook note for 11_manual_corrections.

This public compact repository keeps notebook-export code for workflow auditability. Restricted raw data and derived individual-level panels are not included."""
    build_year = np.array(build_year, dtype=float)
    CI = np.array(CI, dtype=object)
    case_type = np.array(case_type, dtype=object)

    ntl_jump = np.array(meta["ntl_jump_year"], dtype=float)
    pop_jump = np.array(meta["pop_jump_year"], dtype=float)

    if max_year is None:
        max_year = float(years_clcd[-1])

    refined_flag = np.zeros(build_year.shape[0], dtype=bool)

    for i in range(build_year.shape[0]):
        if CI[i] != "L3":
            continue
        if case_type[i] not in allowed_case_types:
            continue
        ...
        proxy = float(min(cand))
        proxy = min(proxy, max_year)

        old = build_year[i]

        # Original notebook comment normalized for the public code archive.
        if np.isnan(old) or old < proxy:
            build_year[i] = proxy
            refined_flag[i] = True

    return build_year, CI, refined_flag
...
def main():
    ...
    # Original notebook comment normalized for the public code archive.
    build_year_ref, CI_ref, refined_flag = refine_L3_with_ntl_pop(
        years_clcd=years_clcd,
        build_year=build_year,
        CI=CI,
        case_type=case_type,
        meta=meta,
        allowed_case_types=("A_CLCD_left", "C_urban_N", "D_NoDate"),
        max_year=years_clcd[-1]
    )

    # Original notebook comment normalized for the public code archive.
    build_year = build_year_ref
    CI = CI_ref

    # Original notebook comment normalized for the public code archive.
    fuse_note = [
        (fn + "|L3_refined") if flag else fn
        for fn, flag in zip(fuse_note, refined_flag)
    ]
