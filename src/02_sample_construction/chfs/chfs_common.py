...
# -*- coding: utf-8 -*-
"""
Common helper functions for CHFS/CFHS household education mechanism scripts.

This file contains generic utilities only. The original wave-specific notebook
logic is preserved in the archived workflow scripts.
"""
...
CHFS_WAVES = [2011, 2013, 2015, 2017, 2019]
...
def safe_log1p(s: pd.Series) -> pd.Series:
    ...
    mask = x >= 0
    out.loc[mask] = np.log1p(x.loc[mask])
    return out


def binary_from_positive_amount(s: pd.Series) -> pd.Series:
    x = pd.to_numeric(s, errors="coerce")
    out = pd.Series(np.nan, index=s.index, dtype="float64")
    out[x == 0] = 0
    out[x > 0] = 1
